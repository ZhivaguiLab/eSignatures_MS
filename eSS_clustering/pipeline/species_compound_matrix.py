...
Supports SBS, DBS, and ID mutation types.
...
def load_compound_grouping_config(config_path):
    """Load compound grouping rules from YAML config."""
    ...
    try:
        ...
        print(f"Loaded compound grouping config: {config_path}")
        return config
    except Exception as e:
        print(f"Warning: Failed to load compound grouping config: {e}")
        return None
 
 
def apply_compound_grouping(compound_name, grouping_config):
    """
    Apply grouping rules to collapse compound variants.
    
    Parameters
    ----------
    compound_name : str
        Original compound name (e.g., "1_human_tumor_connective_SNV_hg38")
    ...
    -------
    str
        Grouped compound name (e.g., "Human_tumor_connective")
    """
    if not grouping_config:
        return compound_name
    
    # Step 1: Try regex rules (in order)
    regex_rules = grouping_config.get('regex_rules', [])
    for rule in regex_rules:
        pattern = rule['pattern']
        replace_with = rule['replace_with']
        
        match = re.match(pattern, compound_name)
        if match:
            # Support capture group substitution
            result = re.sub(pattern, replace_with, compound_name)
            return result
    
    # Step 2: Try explicit mappings (highest priority)
    explicit = grouping_config.get('explicit_mappings', {})
    if compound_name in explicit:
        return explicit[compound_name]
...
def collect_species_compound_data(main_output_dir, acronym_mapping=None,
    ...
    NEW: Supports compound grouping to collapse replicate/variant names.
    ...
    def _normalize(compound):
        # Apply grouping rules FIRST
        if grouping_config:
            compound = apply_compound_grouping(compound, grouping_config)
        
        # Then handle case preference
        key = compound.lower()
        if key not in _case_pref:
            _case_pref[key] = compound
        elif compound != compound.lower():
            _case_pref[key] = compound
    ...
    def _process(sample, source):
        ...
        compound = _normalize(compound)  # Now applies grouping
    ...
    if grouping_config:
        print(f"  (after applying compound grouping rules)")
    return result
...
def main():
    ...
    parser.add_argument("--compound_grouping",
                        help="YAML config for collapsing compound variants (optional).")
    ...
    parser.add_argument("--mutation_type", default="SBS",
                        choices=["SBS", "DBS", "ID"],
                        help="Mutation type being processed (for display purposes).")
    ...
    grouping_config = load_compound_grouping_config(args.compound_grouping)

    print("=" * 60)
    print(f"SPECIES-MODEL vs COMPOUND MATRIX ({args.mutation_type})")
    print("=" * 60)
    print(f"Mutation type:     {args.mutation_type}")
